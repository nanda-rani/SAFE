...
→ If any answer is missing: state uncertainty explicitly and avoid strong labels.
...
═══════════════════════════════════════════════════════════
TRUE_SECURITY_RISK      → ONLY if ALL true: attacker-controlled input confirmed,
                          vulnerable path is reachable, realistic impact in artifact context.
...
HARD GUARDRAIL (also enforced in post-processing code):
• Do NOT assign TRUE_SECURITY_RISK if input_controlled_by_attacker is 'uncertain' or 'no'.
• Do NOT assign TRUE_SECURITY_RISK if reachable_in_artifact_execution is 'uncertain' or 'no'.
...
{{
  "security_label": "<TRUE_SECURITY_RISK|CONTEXTUAL_RISK|HARDENING_RECOMMENDATION|BENIGN_RESEARCH_USAGE|FALSE_POSITIVE>",
  "code_purpose": "...",
  "execution_context": "...",
